...
$IdentityNameWasProvided = [bool]$IdentityName -or [bool]$env:TERRASATCH_MSIX_IDENTITY_NAME
$PublisherWasProvided = [bool]$Publisher -or [bool]$env:TERRASATCH_MSIX_PUBLISHER
$PublisherDisplayNameWasProvided = [bool]$PublisherDisplayName -or [bool]$env:TERRASATCH_MSIX_PUBLISHER_DISPLAY_NAME
...
if ($StoreUpload) {
    if (-not $IdentityNameWasProvided -or -not $PublisherWasProvided -or -not $PublisherDisplayNameWasProvided) {
        throw "StoreUpload requires explicit Partner Center identity values via parameters or TERRASATCH_MSIX_IDENTITY_NAME, TERRASATCH_MSIX_PUBLISHER, and TERRASATCH_MSIX_PUBLISHER_DISPLAY_NAME."
    }
    if ($IdentityName -eq "TerraSatch.Edge.Dev") {
        throw "StoreUpload cannot use the development identity TerraSatch.Edge.Dev. Copy the exact Package/Identity/Name from Partner Center."
    }
}
