...
def resolve_radio_config(
    ...
    if isinstance(radio, dict):
        for key in ("enabled", "mode", "profile", "receivers", "groups", "logical_channel_id"):
            if key in radio:
                base[key] = radio[key]
    ...
    try:
        ...
        resolved = RadioMonitorConfig.model_validate(local_defaults)
...
class RadioMonitorService:
    ...
    def _payload_for(self, capture: RadioAudioCapture, transcript: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "source": f"{self.edge_config.source}-radio-bca-ch{capture.channel:02d}"[:64],
            "channel_id": self.config.logical_channel_id,
