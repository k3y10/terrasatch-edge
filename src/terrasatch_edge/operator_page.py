...
<style>
:root{{color-scheme:dark;--bg:#111317;--surface:#1f2228;--surface2:#292c32;--line:#383c42;--soft:#292c32;--text:#e9e7e2;--muted:#838995;--orange:#f2960d;--positive:#f2960d;--amber:#ee8c2b;--red:#ff625c;--ui:"Segoe UI",Inter,system-ui,sans-serif;--display:"Arial Narrow","Roboto Condensed","Segoe UI",sans-serif}}
...
.step-note{{margin:0 0 16px;color:#c2c6c3;line-height:1.55}}.primary-action{{display:block;width:min(100%,440px);min-height:54px;margin:auto;border:0;border-radius:8px;background:var(--orange);color:#111317;font-weight:700;cursor:pointer;transition:.16s ease}}.primary-action:hover{{background:#ee8c2b;transform:translateY(-1px)}}.primary-action:disabled{{opacity:.7;cursor:wait;transform:none}}
.text-action{{display:block;margin:15px auto 0;padding:3px 5px;border:0;background:transparent;color:var(--orange);text-decoration:underline;text-underline-offset:4px;cursor:pointer}}.text-action:hover{{color:#f3ad45}}
.pairing-panel,.result-panel{{display:none;margin:18px 0 0;padding:16px;border:1px solid var(--line);border-radius:8px;background:var(--surface2)}}.pairing-code{{font:1.25rem ui-monospace,Consolas,monospace;color:var(--orange);letter-spacing:.06em}}.pairing-panel p{{margin:7px 0 14px;color:var(--muted)}}.inline-action{{display:inline-flex;min-height:42px;align-items:center;padding:0 16px;border-radius:7px;background:var(--orange);color:#111317;font-weight:700;text-decoration:none}}
.ready-content{{text-align:center}}.ready-mark{{width:96px;height:96px;margin:4px auto 28px;border:2px solid var(--positive);border-radius:50%;position:relative}}.ready-mark::after{{content:"";position:absolute;left:29px;top:22px;width:24px;height:42px;border:solid var(--positive);border-width:0 5px 5px 0;transform:rotate(45deg)}}
.ready-facts{{display:grid;grid-template-columns:1fr 1fr;gap:0 28px;width:min(100%,680px);margin:32px auto 34px;text-align:left}}.ready-facts div{{padding:17px 4px;border-bottom:1px solid var(--line)}}.ready-facts dt{{color:var(--muted);font-size:.87rem}}.ready-facts dd{{margin:6px 0 0;font-size:1.08rem;font-weight:600;overflow-wrap:anywhere}}.positive{{color:var(--positive)}}.quiet-actions{{display:flex;justify-content:center;gap:42px}}
.result-panel{{max-width:680px;margin:22px auto 0;text-align:left}}.result-row{{display:flex;justify-content:space-between;gap:16px;padding:10px 0;border-bottom:1px solid var(--soft)}}.result-row:last-child{{border:0}}.result-row b{{color:var(--positive);font-size:.75rem;text-transform:uppercase;letter-spacing:.08em}}.result-row.fail b{{color:var(--red)}}
.service-note{{margin:auto auto 0;padding-top:14px;color:#858b87;text-align:center;font-size:.88rem}}#notice{{position:fixed;z-index:20;left:50%;top:18px;transform:translateX(-50%);display:none;width:min(calc(100% - 32px),720px);padding:13px 17px;border:1px solid #765415;border-radius:8px;background:#2a2112;box-shadow:0 14px 40px rgba(0,0,0,.35)}}
...
@media(max-width:720px){{.shell{{padding:20px 16px 16px}}.app-header{{align-items:flex-start}}.brand{{gap:10px}}.brand-mark{{width:50px;height:50px}}.brand-name{{font-size:1.3rem}}.version{{display:none}}.workspace{{margin-top:24px;padding:30px 20px 26px}}.workspace h1{{font-size:2rem}}.lead{{font-size:.95rem}}.setup-step{{grid-template-columns:46px 1fr;gap:12px}}.step-marker{{width:46px;height:46px}}.setup-step:not(:last-child)::before{{left:22px;top:42px}}.step-copy h2{{min-height:46px;font-size:1.08rem}}.check-row{{grid-template-columns:21px 1fr}}.check-row b{{grid-column:2}}.ready-mark{{width:78px;height:78px}}.ready-mark::after{{left:24px;top:18px;width:19px;height:34px}}.ready-facts,.form-grid{{grid-template-columns:1fr}}.quiet-actions{{gap:4px;flex-direction:column}}.service-note{{padding-bottom:4px}}.brand-lockup{{grid-template-columns:68px minmax(0,1fr);gap:10px}}.brand-lockup-kicker{{font-size:.48rem;letter-spacing:.2em}}.brand-lockup-name{{font-size:1.45rem;letter-spacing:-.045em}}.brand-lockup-values{{font-size:.45rem;gap:2px;letter-spacing:.035em}}}}
...
function notify(message,error=false){{const el=document.getElementById("notice");el.textContent=message;el.style.display="block";el.style.background=error?"#351918":"#2a2112";el.style.borderColor=error?"#783b37":"#765415";window.setTimeout(()=>{{el.style.display="none";}},7000)}}
