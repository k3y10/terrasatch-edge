...
The recommended install/startup-task validation is the same local harness with `-Install` from an elevated PowerShell session:

```powershell
.\scripts\qa-windows-msix-local.ps1 -Install
```

If you need to rerun only the install helper against the artifact generated by that harness:

```powershell
.\scripts\install-windows-msix-dev.ps1 `
  -MsixPath ".\release\TerraSatch-Edge_0.2.8_x64.msix" `
  -CertificatePath ".\release\TerraSatch-MSIX-LocalQA.cer"
```
